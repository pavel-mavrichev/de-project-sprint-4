...
class TimestampLoader:
    ...
    WF_KEY = "example_ordersystem_timestamps_stg_to_dds_workflow"
    LAST_LOADED_TS_KEY = "last_loaded_ts"

    def __init__(self, collection_loader: TimestampReader, pg_dest: PgConnect, pg_saver: PgSaver, logger: Logger) -> None:
    ...
    def run_copy(self) -> int:
        ...
        with self.pg_dest.connection() as conn:
            ...
            load_queue = self.collection_loader.get_timestamp(last_loaded_ts, self._SESSION_LIMIT)
            self.log.info(f"Found {len(load_queue)} documents to sync from timestamps collection.")
            if not load_queue:
            ...
            i = 0
            for d in load_queue:
                self.pg_saver.save_timestamp(conn, 
                                            str(d["ts"]), 
                                            d["year"], 
                                            d["month"], 
                                            d["day"],
                ...
                if i % self._LOG_THRESHOLD == 0:
                    self.log.info(f"processed {i} documents of {len(load_queue)} while syncing timestamps.")
